...
int main(int argc, char *argv[])
{
    ...
    strcpy(ack, "Packet 1 Recieved");

    //send acknowledgment to client
    sendto(sockfd, ack, strlen(ack), 0, (struct sockaddr *)&their_addr, addr_len);
    ...
    for (i = 0; i < file_size-1; i++){
        ...
        sprintf(ack, "Packet %lu Recieved", i+2);
        

        sendto(sockfd, ack, strlen(ack), 0, (struct sockaddr *)&their_addr, addr_len);
        printf( "Sent Ack string: %s\n",ack);
    }
