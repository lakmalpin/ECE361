...
uint8_t* serialize_packet(unsigned long total_frag, uint8_t packet_count, int byte_count, char *filename, uint8_t *filedata, int *packet_size){
    uint8_t packet[2000];
    int packet_index;
    ...
    packet_index = sprintf(packet, "%lu", total_frag);
    packet[packet_index] = ':';
    packet_index ++;
...
int main(int argc, char *argv[])
{
    ...
    struct timeval RTTstop, RTTstart;
    //struct timeval ackstart, ackstop;
...
//-------------------------------------------------------------------------------------------------------------
    ...
    unsigned long timeout = 500; //in ms
    unsigned long RTTmeasurement = 0;
    unsigned long a=0;
    for (a = 0; a < total_frag; a++){
        
        

        gettimeofday(&RTTstart, NULL);
        ...
        for (retransmit_attempts; retransmit_attempts < 5; retransmit_attempts++){
            
            //poll waits for data to arrive on the socket until the given timeout
            int num_events = poll(pfds, 1, timeout); 

            if (num_events == 0){
                printf("Ack not recieved, retransmit attempt: %d\n", retransmit_attempts+1);
            }
        ...
        sprintf(ack, "Packet %lu Recieved", a+1);

        if (strcmp(buf,ack) == 0) {
            printf("Packet %d Sent Successfully! \n", a+1);
            RTTmeasurement = (RTTstop.tv_sec - RTTstart.tv_sec) * 1000000 + RTTstop.tv_usec - RTTstart.tv_usec;
            printf("RTT: %lu us\n",RTTmeasurement);
            timeout = RTTmeasurement + 4*(RTTmeasurement/2);
